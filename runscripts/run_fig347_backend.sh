...
MEM_MINOR_PGO_BINARY="${MEM_MINOR_PGO_BINARY:-$PGO_BINS_DIR/univ_cand/gem5_mem-minor.pgo}"
MEM_MINOR_RUBY_PGO_BINARY="${MEM_MINOR_RUBY_PGO_BINARY:-$PGO_BINS_DIR/univ_cand/gem5_mem-minor-ruby.pgo}"
...
parse_bench_info() {
  ...
  local -n out_stdin_ref=$5
  local -n out_mem_ref=$6
  ...
  case ${#bench_parts[@]} in
    ...
    4)
      out_binary_ref="${bench_parts[0]}"
      out_args_ref="${bench_parts[1]}"
      out_stdin_ref="${bench_parts[2]}"
      out_mem_ref="${bench_parts[3]}"
      ;;
    *)
...
if [ "$EVAL_PGOS_MIBENCH" = true ]; then
  ...
  if declare -p MIBENCH_BENCHMARKS_ALL >/dev/null 2>&1; then
    MIBENCH_BENCHMARKS=("${MIBENCH_BENCHMARKS_ALL[@]}")
  else
    echo "ERROR: Shared MIBENCH_BENCHMARKS_ALL is missing. Please source setup/init.sh"
    exit 1
  fi
...
if [ "$EVAL_PGOS_SPLASH_4CORE" = true ]; then
  ...
  if ! declare -p SPLASH_4CORE_TO_BASE_MAP >/dev/null 2>&1; then
    echo "ERROR: Shared SPLASH_4CORE_TO_BASE_MAP is missing. Please source setup/init.sh"
    exit 1
  fi
  ...
  for bench in "${SPLASH_4CORE_BENCHMARKS[@]}"; do
    base_bench="${SPLASH_4CORE_TO_BASE_MAP[$bench]}"
