...
"""Generate the Splash single-core PGO evaluation plot for Figure 7c."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate Splash single-core PGO evaluation bar graph"
    )
    parser.add_argument(
        "--num-iterations",
        type=int,
...
def load_execution_times(csv_file, num_iterations):
    ...
    print(
        f"Loaded {len(df)} execution time records for {len(benchmarks)} Splash benchmarks "
...
def save_legend(labels, colors, output_png):
    ...
    fig = plt.figure(figsize=(max(8.0, 2.6 * len(labels)), 1.4))
    fig.legend(
        handles=handles,
        labels=labels,
        loc="center",
...
def generate_bar_chart(plot_df, output_png, legend_png):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(pivot_df.index, rotation=0, ha="center", fontsize=22)
...
def main():
    ...
    execution_csv = data_dir / "splash_execution_times.csv"
    output_png = figs_dir / "fig7c.png"
    legend_png = figs_dir / "fig7c_legend.png"
    plot_csv = data_dir / "fig7c_data.csv"

    print("=" * 80)
    print("Splash Single-Core PGO Evaluation Bar Graph Generation")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("Splash Single-Core PGO Evaluation Bar Graph Generation Complete")
    print("=" * 80)
    print(f"  - {plot_csv}")
    print(f"  - {output_png}")
