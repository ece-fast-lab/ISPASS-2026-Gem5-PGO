    ...
    if [ -f "$pgo_binary" ]; then
        ...
        if cp "$pgo_binary" "$dest_binary"; then
            ...
            if [ "$dest_size" -eq "$src_size" ]; then
                ...
                # Collect and save optimization remark files
                echo "[$bench] Processing optimization remarks..."
                opt_temp_dir="$PGO_BINS_DIR/$bench/opt-${bench}-pgo-temp"
                ...
                if find "${build_dir}" -name "*.opt.yaml" -type f | grep -q .; then
                    echo "[$bench]   Collecting optimization remark files..."
                    find "${build_dir}" -name "*.opt.yaml" -type f -exec cp {} "$opt_temp_dir/" \;
                    num_files=$(find "$opt_temp_dir" -name "*.opt.yaml" -type f | wc -l)
                    echo "[$bench]   Collected $num_files optimization remark files"

                    # Compress with pigz (parallel)
                    if ! cd "$PGO_BINS_DIR/$bench"; then
                        echo "[$bench]   Error: Failed to cd to $PGO_BINS_DIR/$bench, skipping opt.yaml processing"
                        rm -rf "$opt_temp_dir"
                        cd "$GEM5_DIR"
                    else
                        tar_name="opt-${bench}-pgo.tar.gz"

                        echo "[$bench]   Compressing with pigz (parallel)..."
                        if ! tar -I pigz -cf "$tar_name" -C "$opt_temp_dir" .; then
                            echo "[$bench]   Error: Failed to compress optimization remarks"
                            rm -rf "$opt_temp_dir"
                            cd "$GEM5_DIR"
                        else
                            # Create destination directory on NAS
                            nas_dest="/fast-lab-share/hnpark2/gem5_pgo/opt_remarks/$bench"
                            mkdir -p "$nas_dest"

                            # Transfer to NAS using rsync
                            echo "[$bench]   Transferring to NAS..."
                            if ! rsync -avh --no-owner --no-group --progress "$tar_name" "$nas_dest/"; then
                                echo "[$bench]   Error: Failed to transfer to NAS"
                                rm -rf "$opt_temp_dir"
                                rm -f "$tar_name"
                                cd "$GEM5_DIR"
                            else
                                # Extract on NAS with pigz (parallel)
                                echo "[$bench]   Extracting on NAS with pigz (parallel)..."
                                nas_extract_dir="$nas_dest/opt-pgo"
                                mkdir -p "$nas_extract_dir"
                                if ! tar -I pigz -xf "$nas_dest/$tar_name" -C "$nas_extract_dir"; then
                                    echo "[$bench]   Error: Failed to extract on NAS"
                                    ...
                                    echo "[$bench]   Successfully saved $num_files optimization remark files to $nas_extract_dir"
                                fi
                            fi
                        fi
                    fi
                else
                    echo "[$bench]   Warning: No optimization remark files found in ${build_dir}"
                    rm -rf "$opt_temp_dir"
                fi
            else
