...
#  "605.mcf_s                  | $SPEC_BUILT_DIR/605.mcf_s/run/run_base_refspeed_$RUN_LABEL.0000/mcf_s_base.$RUN_LABEL                    |           $SPEC_BUILT_DIR/605.mcf_s/run/run_base_refspeed_$RUN_LABEL.0000/inp.in | 8GiB"
...
for entry in "${benchmarks[@]}"; do
    # Split the entry using '|' as the delimiter
    ...
    # Run gem5 using environment variables and values from each benchmark entry
    "$GEM5" -r --outdir="$RESULTS_RUNDIR_DIR/ckpt-gen/${bench}" "$GEM5_CONFIG" \
      --checkpoint-dir "$CHECKPOINT_BASE_DIR/${bench}" --binary "$binary" --args="$args"  \
      --mem-size "$mem" \
      --weights "$SIMPOINT_OUTPUT_DIR/${bench}_simpoint/${bench}.weights" --simpts "$SIMPOINT_OUTPUT_DIR/${bench}_simpoint/${bench}.simpts"

    echo "Checkpoint for $bench generated successfully."
done
